#!/usr/bin/env python3
"""
Scrape child care inspection reports — Vancouver Coastal Health.

VCH publishes routine + follow-up inspection reports through a searchable
database ("Inspection Reports website") covering Vancouver, Richmond, North
Shore, Sunshine Coast, and Sea to Sky. Start here:
  https://www.vch.ca/en/child-care-facility-inspections

HOW TO USE THIS SCRIPT (honest version):
This is a working framework, not a finished scraper. I could not inspect the
live VCH site while writing it, so the two things marked ADJUST below need
one pass of confirmation against the real pages:

  ADJUST 1 — LISTING_URLS: the actual URL(s) of the inspection report
             listing/search results. Open the VCH inspection database in your
...
Run:  python3 scripts/scrape_vch.py
...
# CONFIRMED: the province publishes this exact URL in the HA_FAC_INSPEC_RPTS
# column of childcare_locations.csv for every VCH-region facility.
# It's a table view — check whether it paginates via query string (?page=2) or
# loads rows from a JSON endpoint (DevTools > Network); if JSON, point fetch()
# at that endpoint instead and parsing becomes trivial.
LISTING_URLS = [
    "https://inspections.vcha.ca/ChildCare/Table",
]
...
def main():
    if not LISTING_URLS:
        ...
        print("Open the VCH inspection database, find the listing URL pattern,")
        print("and add it to LISTING_URLS at the top of this script.")
        print("Start here: https://www.vch.ca/en/child-care-facility-inspections")
        return
    ...
    if scraped:
        attach_inspections(scraped, "Vancouver Coastal Health")
    else:
